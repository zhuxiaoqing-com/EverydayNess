...
public final class LobbyLoadBalancerLogic {
    public LobbyConnCandidate selectLeastLoadedConn() {
        RpcResult<SOnlineConnCandidate> result = OnlineRoutingRpcProxy.callSelectLeastLoadedConn(null);
        if (!result.isSuccess() || result.getValue() == null) {
            return null;
        }
        SOnlineConnCandidate candidate = result.getValue();
    ...
    public LobbyPlayerCandidate selectLeastLoadedPlayer() {
        RpcResult<SOnlinePlayerCandidate> result = OnlineRoutingRpcProxy.callSelectLeastLoadedPlayer(null);
        if (!result.isSuccess() || result.getValue() == null) {
            return null;
        }
        SOnlinePlayerCandidate candidate = result.getValue();
